...
// promises that made committing them possible at all: no live path in either
// file, and no drift from `deploy/contract.json`, which is the list of what this
// app reads. A variable the launcher sets that the contract does not declare is
// a variable that does nothing.
//
// It is a text check on two shell scripts, and it fails closed on purpose: an
// unrecognised `$env:` assignment is a complaint rather than a shrug.
...
/**
 * What a live path looks like. A drive-letter absolute path is the whole family:
 * it is the only way to name a location on this machine that another machine
 * would not resolve. The rest would otherwise slip through as relative
 * fragments.
 */
 ...
 * Empty, and it should stay that way: it is a named list so that adding one is a
 * decision somebody writes a reason beside.
 */
...
/**
 * Live paths in a source, comments included. Deliberately not comment-stripped,
 * unlike the variable scan above: a comment quoting somebody's directory is
 * still that directory written down in this repository.
 */
export function livePathsIn(source) {
  return LIVE_PATHS.filter((one) => one.pattern.test(source)).map((one) => one.what)
}

/** Returns a list of complaints, empty when there is nothing wrong. */
export function launcherProblems({ launcher, entry, contract }) {
  ...
  // `deploy/check-config.mjs` reads the environment the launcher built and says
  // which of the contract's refusals it would meet. That coverage would be lost
  // silently if the call were dropped.
