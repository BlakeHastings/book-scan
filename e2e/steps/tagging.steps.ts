/**
 * Saying what a book is, on the check-the-details screen.
 *
 * Two kinds of step here and the difference is the point of the file: the
 * ones that drive the panel act like a person, and the ones that check go to
 * the database, since a person's tag surviving a save that states a genre is
 * not something a screen can be asked.
 */
 ...
 *
 * Through the real route rather than by writing a row: what the scenario is
 * about is what happens the second time somebody names it, so the first time
 * has to have been real. It hangs off a book rather than using
 * `POST /api/tags`, on purpose: the first naming should be the door the
 * check-the-details screen uses, not the one the tags screen does.
 */
...
When('I open my tags', async ({ page }) => {
  ...
  // The row above the books, which is where choosing a tag lives.
 ...
 * rather than an economy: this panel is meant to be shared across the doors
 * that use it, and a step file that needed a second set of selectors for it
 * would be saying it had not.
 */
 ...
 * not through the panel. A genre is stated only when a source did or a person
 ...
 *
 * The source is in the table on purpose: "Dune carries a comic book tag"
 * would pass on a tag the catalogue happened to send, when what matters is a
 * person's word surviving a save that states a genre.
 */
