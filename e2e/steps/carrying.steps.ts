 ...
 *
 * Every claim these make ends in the database, and it is always the same
 * claim said two ways: where each book is recorded did not change. That is
 * the one thing withdrawing an intention must never touch, and it is
 * invisible from a screen that has correctly stopped drawing the work.
 */
...
/** Read off the tile rather than off the list behind it: the two can disagree, and a scenario that only opened the list would never see it. */
...
/**
 * The wait is on the top bar rather than on a heading, because a job of one book
 * skips the list and lands on the trip: `CarryScreen` redirects when `moving` is
 * 1, so the screen that arrives says "One book to carry" instead of "Books to
 * carry". Waiting for one of the two names would make this step depend on how
 * much work there happens to be.
 */
...
/** The primary button rather than the row: it names where to start, "Start at 5A" or "Carry on at 5A" on a list somebody is part way through. */
...
/** Writes nothing: a book in transit gets no row, so this is a navigation and there is nothing to unwind if the phone locks halfway across the room. */
...
/**
 * A bounded loop rather than a wait for a count: the armful is however many
 * books the trip had, and an unbounded loop would read a broken flow as a hung
 * test rather than a failure. Ends when the trip does, on "Carried".
 */
...
/** Substrings rather than whole screens, the way the plan's own step reads them: the claim is that each sentence is in front of the person. */
...
/**
 * Driven through `PATCH /api/books/:id/location`, the route the carrying screens
 * press, rather than pressed: the scenario is about what happens to the books
 * that were **not** carried, and the carrying journey has its own scenarios.
 */
...
/** The dialog's own button is asked for inside the dialog: it says the same words as the one that opened it, so the press has to name where it is looking. */
...
/** A list emptied by moving books draws exactly the same as one that emptied honestly, so this checks the database instead. */
