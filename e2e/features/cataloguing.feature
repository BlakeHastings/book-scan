Feature: Cataloguing a book from its cover
  ...
  Scenario: The next photograph does not land on the book just shelved
    Shelving a book finishes with it: the tab bar is on the screen after
    shelving, and pressing Scan there is what somebody with the next book
    already in hand does.

    This is asserted against the database and not the screen, and all three
    sides are photographed so that there is a spine to lose.
