 ...
 *
 * Postgres only. `tag` and `book_tag` arrive in a migration, and
 * migrations exist only for Postgres; nothing here is dialect-specific on
 * purpose, but nothing tests it on SQLite either, since there is no
 * SQLite database in this project with these tables.
 *
 * "Everything under `genre`" is asked as
 * `slug >= 'genre/' AND slug < 'genre0'` rather than `slug LIKE 'genre/%'`:
 * a range over a byte-ordered column (`COLLATE "C"`) is an index range on
 * `tag_slug_key`, while a `LIKE` whose pattern arrives as a parameter
 * degrades to a scan of the vocabulary, silently, since the answers are
 * identical and only the plan changes. `tag-repository.test.ts` reads the
 * plan back out of Postgres rather than trusting this paragraph.
 */
...
export class DrizzleTagRepository implements TagRepository {
  ...
  /**
   * `ON CONFLICT DO NOTHING`, then read it back. Two statements rather
   * than an upsert: an upsert would rewrite the label every time a
   * catalogue spelled a heading differently, and the label is a person's
   * to change. The conflict target is the slug, so two callers racing to
   * define one tag both end up with the row the first of them wrote.
   */
  ...
  /**
   * One statement per application, each finding its tag by slug. The
   * `select` inside the insert keeps the id out of this layer's callers: a
   * handler names a slug, never a row id. A slug nobody has defined writes
   * nothing, so that is turned into an error here.
   */
   ...
   * `source` is part of the `where`, not a filter applied afterwards,
   * which is what makes "a lookup may retract its own tags and no others"
   ...
   * `book_tag.tag_id` is `ON DELETE CASCADE`, so an unguarded delete here
   * would silently take the tag off every book carrying it rather than
   * failing; the guard belongs in the statement rather than in a caller
   * that might skip it. A caller that gets `false` asks the database why
