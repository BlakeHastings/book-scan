 ...
 * Drizzle never sees a connection. See `infrastructure/db/query.ts` for why.
 *
 * Every write here is monotone, which is why none of them takes a lock. A
 * crop arrives, a hash arrives, `examined` goes from false to true, and no
 * statement in this file can take any of the three back, so two callers
 * racing agree whatever order they land in. That matters because crop
 * passes over one book overlap routinely: one is fired after a save and
 * the other is the backfill loop. The fact lives on the photograph it is
 * about rather than in a list shared by several, which is what makes that safe.
 */
...
export class DrizzleCaptureRepository implements CaptureRepository {
  ...
  /**
   * Many books' photographs in one statement, ordered exactly as `of`
   * orders one book's. The ordering matters: `Photographs.of` sorts newest
   * first with a stable sort, so the tiebreak between two photographs
   * taken in the same second is the order the rows arrive in. Reading
   * them by id here is what makes the answer for a book identical whether
   * it was asked for on its own or alongside a hundred others.
   */
  ...
  /**
   * One upsert per photograph, keyed on the book and the file. `kind` and
   * `taken_at` are deliberately not in the `DO UPDATE`: a photograph is of
   * what it is of and was taken when it was taken, and a later save
   * restating the same file should not move either. The three that are
   * there each only move one way, which is what makes this safe to run
