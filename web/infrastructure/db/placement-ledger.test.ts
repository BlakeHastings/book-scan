/**
 * The claim this step rests on, checked book by book: the ledger, replayed,
 * reproduces `books.location` for every book, and `books.current_area_id` agrees
 * with the ledger for every book. Each catalogued book is folded out of
 * `book_placement` by `domain/placement/ledger.ts`, turned back into a label by
 * `labelFor`, and compared with the column it was built from.
 *
 * That comparison is possible only while `books.location` still holds anything,
 * and `0024` drops it. So every database here stops at `0015`, through
 * `migrationsThrough`, which is where the migration this file is about stops.
 * The comparison over the migrated catalogue is `placement-cutover.test.ts`.
 *
...
/**
 * The same books and boundaries `placement-backfill.test.ts` uses, deliberately:
 * the two steps are asked the same question about the same arrangement, so a
 * disagreement between them is a disagreement about the model rather than about
 * which catalogue was chosen.
 */
 ...
 * standing at the shelves would have typed in. Computed with `layoutRange`, the
 * function the running app draws its shelves with, so these are the locations a
 * real catalogue accumulates rather than ones invented to be easy to parse.
 */
 ...
 * itself and would hand back a database that had already had the ones under
 * test.
 */
async function catalogueOf(
  ...
  // One statement rather than one per book: a round trip per book is hundreds of
  // them per database and a dozen databases across a file.
  if (books.length) {
    await pool.query(
 ...
 *
 * Deliberately made in TypeScript through the domain fold rather than in SQL:
 * `0015` and `projectionDisagreements` are both SQL, and two SQL statements
 * written from the same paragraph can agree with each other and be wrong
 * together. This one reads the rows and folds them with `standingOf`, which is
 * the definition the schema's check constraint is written from.
 *
 * Checked out, withdrawn, queued and discarded books are compared against
 * nothing rather than against `location`, each an exclusion this model makes on
 * purpose: a checked out book holds no position though the column still names
 * the plank it came off, a withdrawn one has left the collection, and the rest
 ...
 * for. Stopped at `0015` rather than migrated to latest, because `0024` drops
 ...
 *
 * One database for all of them, and built in a hook. `vitest.config.ts` raised
 * `hookTimeout` for the queue of `CREATE DATABASE`s the suite makes against one
 * container and deliberately left `testTimeout` alone, so a database made in a
 * hook waits its turn and one made in a test body fails at five seconds.
 *
...
describe('where every book is, becoming rows', () => {
  ...
  beforeAll(async () => {
    ...
    // drops the column this file compares against.
    await migrationsThrough(pool, BACKFILL)
  }, 60_000)
  ...
  it('replays to exactly what books.location says, book by book', async () => {
     ...
     * away. The one book that disagrees is the one recorded on a plank the
     * furniture does not have, which the ledger has no row to point at.
     * Asserting the exact list keeps the limit visible: if a second kind of book
     * ever stops replaying, this fails with its title in the message.
     */
    ...
    // Unchanged because the only column this migration writes to `books` is
  ...
  it('lands every one of them on a plank that exists, across both ranges', async () => {
    ...
    // `placement-backfill.test.ts` asserts the rules produce. 2B is missing on
    // purpose: two separators on one anchor leave an area with no books on it.
    const labels = labelsOf((await furnitureIn(pool)).order)
  ...
  it('writes not one assigned row, because the rules have not run', async () => {
    /*
     * `assigned` is what the rules want, and the rules are TypeScript: a
     * migration does not reimplement them in SQL, and a backfill that wrote one
     * per book would be a row per book saying nothing changed.
     *
     * Two books are not placed: one nobody has placed and one recorded on a
  ...
  it('takes a checked out book off the shelf, and says so in two rows', async () => {
    ...
    // `reviewShelving` excludes such a book from the misfile list for the same
    // reason.
  ...
  it('counts a book recorded on a plank the furniture does not have, and places the rest', async () => {
     ...
     * Refusing would refuse a catalogue the app already handles, so it is
     * counted and named instead, and the book carries no placed row.
     */
  ...
  it('is not run twice on a database that has already had it', async () => {
    ...
    // Run again by hand: a migration somebody is not sure finished should be
    // safe to set going again. Without the guard a second run would double every
    // book's history and the fold would still look right.
    await pool.query(migrationSql(BACKFILL))
...
describe('a catalogue with nothing in it to record', () => {
  ...
  beforeAll(async () => {
    ...
    // The case every developer, every CI run and every end to end run takes: a
    // database created from nothing rather than adopted.
...
describe('a catalogue this migration will not finish on', () => {
  ...
  it('refuses when not one recorded location names an area', () => {
    /*
     * A person mistypes a location now and then; all of them at once is this
     * migration failing to read a label, and without the guard it would add up
...
describe('the checks, proving they can fail', () => {
  ...
  // one hook because a `CREATE DATABASE` in a test body fails at five seconds
  // when the suite is under load.
  beforeAll(async () => {
  ...
  it('names the book when one ledger row is moved to another plank', async () => {
    /*
     * A book the ledger has one plank away from where the catalogue has it.
     * Nothing throws, the book still has a place, and a shelf drawn from the
     * projection would be wrong by one run of books.
     */
...
describe('running the rules over the ledger', () => {
  ...
  // Three, and in a hook, for the reason the tests above are: each of these
  // writes to the ledger, so they cannot share one database.
  beforeAll(async () => {
  ...
  it('writes an assignment only where the rules disagree with the room', async () => {
    /*
     * The seeded catalogue is filed exactly as its rules would file it, so a
     * correct engine writes nothing at all and the ledger is the same size
     * afterwards. An engine comparing the wrong two things would write a row per
     * book, which is the flood that makes a ledger useless as history.
     */
  ...
  it('writes one for a misfiled book, and does not write it again', async () => {
     ...
     * sort key says, so one row is written.
     *
