 ...
 *
 * The migration's whole job is to answer, once, the question every reader
 * of `outstanding_move` used to ask on every read: which plank does `4B`
 * name, and write the answer down so nobody has to ask again.
 *
 * So the claim it has to make is that it answers exactly what the reader
 * answered. `areaForRecordedLabel` was the reading that existed the moment
 * before this ran: `parseLocation` and `areaIndex` for the address, then a
 * lookup that reaches a plank taken off the face, since the move that
 * wrote the receipt is what retired it. That function is gone with its
 * last caller, and this is where its behaviour is pinned instead, spelled
 * in SQL and asked of every shape of furniture it distinguished: a plank
 * on a face, a plank retired with nothing standing in its place, a plank
 * retired with a live one that has taken its address, a plank on a piece
 * that has come off the floor, two pieces on one number, and an address
 * that names nothing at all.
 *
 * The third of those is the one case the migration cannot get right for a
 * row written before it: an address alone does not say which of two rows
 * reading `1B` a receipt meant, and the reader it is replacing answered the
 * live one, so it answers the live one too. Every receipt written after
 * this carries the id the move recorded and never comes near this question.
 *
 ...
 * The catalogue is migrated to latest first and the receipts are written
 * after, rather than being stopped at `0029` and migrated forward: `0023`
 * builds a `TEMP TABLE ... ON COMMIT DROP`, which does not survive being
 * handed statement by statement to a pool. The two column additions are
 * skipped and everything after them runs exactly as Drizzle's migrator
 * would. The rows below carry no ids to begin with, which is the state
 * the migration meets.
 */
...
beforeAll(async () => {
  ...
  // `0013` seeds bookcases 1 and 4 with planks on them, so a fixture of
  // this file's standing on one of those numbers would answer somebody
  // else's address. The furniture below stands at 11 and up.
...
it('prefers the plank on the face where a retired one shares its address', async () => {
  // The reading being reproduced: two rows answer to `1B`, the migration
  // has only the address to go on, and it gives the same answer the code
  // it replaces gave. A receipt written since carries the id the move recorded.
