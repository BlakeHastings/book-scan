...
/**
 * Lay a run out from the first plank of the first bookcase. `layoutRange` has no
 * default start, and nothing here has a rule set to ask, so the start is stated
 * once and every expectation below reads in the addresses it produces.
 */
...
describe('labels', () => {
  ...
  it('reads a plank letter back to the plank it names', () => {
    // Asserted over the whole round trip rather than at three points, because
    // bijective base 26 goes wrong at exactly the boundaries a handful of
    // examples step over.
  ...
  it('has no index for a bookcase with no plank on it', () => {
    // `S4` parses as a location and names no plank, so it is not plank A.
...
describe('layoutRange', () => {
  ...
  it('lets a shelf simply grow when a book is inserted into it', () => {
    // A thin book may well fit, so nothing is displaced automatically; only a
    // person can say otherwise.
...
describe('groupByShelf', () => {
  ...
  /**
   * A line names the boundary it removes, so it belongs above the heading of the
   * area that boundary opens.
   */
...
describe('a cascade across several shelves', () => {
  ...
  it('carries a chain into a new bookcase when asked', () => {
    ...
    // past it comes along rather than staying in bookcase one.
...
describe('boundaryMove', () => {
   ...
   * test can assert on the labels that come out rather than on the edit that was
   * planned.
   */
  const carry = (
    books: { id: number; sortKey: string }[],
  ...
  it('drops the boundary when the area it starts has nothing left after it', () => {
    // C is alone on the last plank, so once it moves back no book is left for
    // that boundary to be anchored to and it goes.
    const books = run('ABC')
    const separators = [sep(1, 'C')]
  ...
  it('leaves a bare plank a later overflow can fill, and name correctly', () => {
    // An empty plank has no books to name it, so it is absent from the groups.
    // An overflow that read its destination off "the next group" would send a
    // book to the plank after the bare one and record it there.
    const books = run('ABC')
    const separators = [sep(1, 'B'), sep(2, 'C')]
    const { separators: after } = carry(books, separators, 2, 'previous')
    expect(labels(books, after)).toEqual(['1A', '1A', '1C'])
  ...
  it('sends the first book of 2A back to the last area of bookcase 1', () => {
     ...
     * boundary is only where that sequence breaks across furniture. So it is the
     * bookcase break that moves, not a plank break, and everything after it
     * stays on bookcase 2 at the area it was on.
     */
    ...
    /*
     * F was the only book on 2A, so 2A is now empty and has no books to name it.
     * Nothing shuffles up to fill it: the books past the break are not disturbed.
     */
  ...
  it('still refuses at the two ends of the range, not at every bookcase', () => {
    // Making new furniture is what declaring a plank full is for, so the refusal
    // is at the start and end of the run, not at a break between two bookcases
  ...
  it('writes the same boundary an overflow of the same shelf would', () => {
    ...
    // question, so writing different things down would have one undo the other.
    const books = run('ABCD')
    const separators = [sep(1, 'C')]
...
describe('carryOn', () => {
  ...
  it('sends the book in your hand on when it belongs at the end', () => {
    ...
    // The book moves and nothing already shelved does.
  ...
  it('makes the plank when the book is last in the whole run', () => {
    // The end of the last area of the last bookcase: nothing to displace and
    // nowhere to displace it to, so the plank gets made.
    const books = run('AB')
  ...
  it('is never reached for a book that lands first on a plank', () => {
    /*
     * A boundary is anchored to the sort key of the first book on its plank, so
     * a book landing on that plank sorts at or after that anchor. There is no
     * key that lands first on a plank whose anchor is a book still on it.
     */
  ...
  it('declines when an orphaned anchor does put it first, leaving the cascade', () => {
     ...
     * longer on it. Going back to the previous plank is not the answer there,
     * because after a carry the anchor is the book in hand, so treating "first"
     * as "go back" would undo the hop just made and ask the same question again.
     */
    const books = run('AD')
