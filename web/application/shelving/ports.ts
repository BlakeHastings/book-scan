 ...
 * interfaces it owns.
 *
 * These are ports: the arrow points inwards, so the layer that stores
 * separators depends on this file rather than this file depending on it.
 * Nothing here imports a driver, a query builder or a connection, and that is
 * the property `npm run lint:layers` checks.
 *
 * Only the shelving furniture is here, which is `separators` and the
 * outstanding moves that name them. Books, captures and the rest still go
 * through `Store` and `CaptureQueue`.
 */
...
export interface SeparatorRepository {
  ...
  /**
   * Point an existing boundary at a different book. A boundary is anchored to a
   * sort key, and moving one is re-anchoring it, not editing a column.
   */
   ...
   * `reanchor` in a loop.
   *
   * A boundary move that empties an area leaves two boundaries on one anchor,
   * and where a boundary sits in the run is decided by its anchor. Applying the
   * first change on its own re-sorts the run under the second, which then has
   * nothing left to do. The set is one edit, so it is written as one.
   */
   ...
   *
   * The whole act, not the row: removing a boundary hands that area's books to
   * the area in front and records where they went, so this method means the
   * retirement, the renumbering and the `assigned` row per book. There is one
   * implementation and it is the same function `DELETE /api/areas/:id` calls.
   *
...
/**
 * What became of the area a boundary opened. A refusal is worded for a person
 * and carries the status a route should answer with. Nothing is written when one
 * comes back.
 */
 ...
 * nobody picked up.
 *
 * What it carries is what the move changed, not what the shelves look like now.
 * Asking for the opposite boundary move instead reads the shelves as they are
 * and answers with where the rules would put the book today, and after a move
 * that emptied an area those are two different planks.
 */
export interface OutstandingMove {
  ...
  to: string
  /**
   * Which plank `from` was a rendering of, and which `to` was.
   *
   * The address and the identity are two fields because they answer two
   * questions, and only the second one keeps answering. A label is derived from
   * where a piece stands and where a plank stands on it, and a boundary write
   * moves both: taking an area off a face renumbers the areas after it, so the
   * row that read `1C` reads `1B` afterwards. Anything asking "is this the same
   * plank" reads these; anything asking "what did this say at the time" reads
   * the two above.
   *
  ...
  /**
   * Boundaries the move took out, to be made again exactly as they were. A move
   * removes a boundary only when the book it re-anchors to would be past the end
   * of the run. Their ids do not survive the deletion, which is why this holds
   * whole separators rather than ids.
   */
  recreate: NewSeparator[]
}

/**
 * Where outstanding moves are kept. One row per book at most: a second move
 * made before anybody has carried it is the same book, still off the plank the
 * catalogue records, one boundary further along, so `record` merges.
 */
export interface OutstandingMoveRepository {
   ...
   * for this book.
   *
   * Merging keeps the older anchor for a boundary named twice, and the older
   * `from` with the older `fromArea`, so what is stored stays the arrangement as
   * it was the last time this book and its shelf agreed. The two `from` fields
   * move together because they are the address and the identity of one plank,
   * and keeping one older than the other is how a receipt comes to name two
   * places.
   */
 ...
 * `rangeLock` in `web/server/shelves.ts` for the one namespace in use.
 */
