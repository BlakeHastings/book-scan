...
describe('bookStillInHand', () => {
  ...
  test('a capture opened from the queue is not in hand, even unsaved', () => {
    // The book was never physically picked up, so it must not survive the trip and receive the next book's photo.
  ...
  test('a catalogued book flagged as from the queue is still not in hand', () => {
    // Should not arise in practice (openBook always clears fromQueue), but either signal being "not in hand" is enough.
